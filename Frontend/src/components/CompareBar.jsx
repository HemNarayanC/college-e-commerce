...
export default function ProductComparison() {
  ...
  return (
    <div className="p-4 h-screen flex flex-col">
          ...
          {products.map((product) => (
            <div key={product.id} className="border p-4 rounded">
              <img
                src={product.image}
                alt={product.name}
                className="mx-auto mb-2 h-86 w-66"
              />
              <h2 className="text-lg font-semibold">{product.name}</h2>
              <button
                className="mt-2 px-4 py-1 bg-gray-200 rounded hover:bg-gray-300 transition-colors duration-200"
        ...
        <>
          ...
          >
            ...
            <button
              ...
              className="px-6 py-2 bg-blue-500 text-white rounded-lg flex items-center gap-2 hover:bg-blue-600 transition-colors duration-200 shadow-md hover:shadow-lg w-42"
              onMouseEnter={handleBarInteraction}
            >
              <span role="img" aria-label="eye" className="text-lg">
                <FaEye />
              </span>
              <span>|</span>
              <span>Compare</span>
          ...
          {/* Floating Icon when CompareBar is hidden */}
          <div
          ...
          >
            <button
              ...
              className="bg-blue-500 text-white p-4 rounded-sm shadow-lg hover:bg-blue-600 transition-colors duration-200 flex items-center gap-2 group"
            >
              <span role="img" aria-label="eye" className="text-xl">
                <FaEye />
              </span>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex justify-center items-center bg-black/30 backdrop-blur-md z-50 transition-all duration-300">
          <div className="bg-white max-w-4xl w-full p-8 rounded-2xl shadow-2xl relative transform transition-all duration-300 scale-100 hover:scale-[1.02]">
            <button
              ...
              className="absolute top-4 right-4 text-lg bg-gray-100 hover:bg-gray-200 rounded-full w-8 h-8 flex items-center justify-center transition-colors duration-200"
            >
              <IoMdClose />
            </button>
